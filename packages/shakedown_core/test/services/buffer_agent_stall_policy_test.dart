...
void main() {
  test('returns 10 seconds for visible web playback', () {
    expect(
      BufferAgentStallPolicy.stallThreshold(isWeb: true, isAppVisible: true),
  ...
  test('returns 20 seconds for hidden web playback', () {
    expect(
      BufferAgentStallPolicy.stallThreshold(isWeb: true, isAppVisible: false),
  ...
  test('returns 20 seconds for visible native playback', () {
    expect(
      BufferAgentStallPolicy.stallThreshold(isWeb: false, isAppVisible: true),
      const Duration(seconds: 20),
    );
  });
}
