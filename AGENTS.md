...
* **Channel Separation:** Never emit the same message in both `commentary` and `final`; `commentary` is for progress only, and `final` must be a distinct closeout response.
