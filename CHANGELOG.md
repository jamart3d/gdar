...
### Fixed
- **Testing**: Resolved redundant `unused_import` lint in `apps/gdar_mobile`.
- **Maintenance**: Successfully completed full monorepo-wide health check with 258/258 tests passing across all targets.
- **Hygiene**: Cleaned workspace root of temporary build logs and diagnostic artifacts to maintain monorepo hygiene.
...
### Added
- **Core**: Synchronized monorepo auto-approval policies and orchestration rules for zero-friction production releases.
- **Maintenance**: Updated `.agent/rules/` to enforce stricter adherence to the Zero-Friction Mandate and Proxy Confirmation protocols.

## [1.3.27+237] - 2026-03-27

### Added
- **Core**: Implemented `markAsPlayed` on track start in `AudioProvider` to automatically update catalog history when a track begins.
- **Maintenance**: Formalized project-wide "Agent Ethics" with numbered rules 1-3 (No making shit up, No assumptions, Zero-friction autonomy).
...
### Fixed
- **TV Enhancement**: Finalized formatting and layout stability for the real-time developer monitor (SS indicator).
- **Maintenance**: Orchestration of the automated /deploy pipeline across the monorepo.
