...
1. **Verify Tool**: Run `where.exe jules` (Windows) or `which jules` (ChromeOS) to ensure the `jules` binary is linked and available in your environment.
2. **Check Status**: Ensure `git status` is clean before handoff.
