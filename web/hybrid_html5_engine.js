...
(function () {
    ...
    class Queue {
        ...
        gotoTrack(idx, playImmediately = false) {
            ...
            if (playImmediately) {
                this.play();
                if (this.props.onStartNewTrack && this.currentTrack) this.props.onStartNewTrack(this.currentTrack);
            }
        }
    ...
    const api = {
        ...
        syncState: function (index, position, shouldPlay) {
            ...
            if (shouldPlay) {
                _queue.play();
                if (_queue.currentTrack) _queue.currentTrack.seek(position);
            } else {
                if (_queue.currentTrack) _queue.currentTrack.seek(position);
                _queue.pause();
            }
